...
def run_gmail_registration():
    ...
    try:
        ...
        success_message = "Authentication successful! You can now close this tab and return to your RENA Dashboard which has been unlocked."
...
def create_google_meeting(summary, start_time_iso, duration_minutes=30):
    ...
    try:
        ...
        event = {
            'summary': summary,
            'description': 'Meeting created via RENA AI Assistant',
    ...
    def join_google_meet(self, meet_url, record=True, db=None, meeting_id=None):
        ...
        try:
            with sync_playwright() as p:
                ...
                try:
                    page.wait_for_selector('button[aria-label="Leave call"]', timeout=300000) # Wait 5 mins for admission
                    print("✅ ADMITTED TO MEETING!")
                    
                    # UPDATE DB STATUS TO CONNECTED HERE
                    if db and meeting_id:
                        joined_time = datetime.now(timezone.utc).isoformat()
                        db.set_meeting_bot_status(meeting_id, "CONNECTED", joined_at=joined_time, status_note="In meeting - Wait 30s before speaking")
                        print(f"📢 DB Status Updated: CONNECTED at {joined_time}")
                        
                except:
                    print("⚠️ DEBUG: Timed out waiting for admission (or selector changed).")
            ...
            if db and meeting_id and self.recording_path and os.path.exists(self.recording_path):
                ...
                try:
                    ...
                    print("🎙️ Stage 1/4: Initializing Transcription Engine...")
                    ...
                    print("🔊 Stage 2/4: Running Faster-Whisper Transcription...")
                    generator.transcribe_audio()
                    
                    print("👥 Stage 3/4: Activating NeMo Speaker Diarization...")
                    generator.perform_diarization()
                    
                    print("🧠 Stage 4/4: Generating AI Summary, MOM & Action Plans...")
                ...
                for event in upcoming:
                    ...
                    if -5 <= time_diff <= 60 and not meeting_ended:
                        ...
                        if not meet_url and 'description' in event:
                            ...
                            match = re.search(r'https://meet\.google\.com/[a-z]{3}-[a-z]{4}-[a-z]{3}', desc)
                            if match:
                                meet_url = match.group(0)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python rena_bot_pilot.py [URL], --manual, or --autopilot")
